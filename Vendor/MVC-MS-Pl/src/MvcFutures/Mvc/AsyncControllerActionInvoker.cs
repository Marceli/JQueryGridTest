namespace Microsoft.Web.Mvc {
    ...
    public class AsyncControllerActionInvoker : ControllerActionInvoker, IAsyncActionInvoker {
        ...
        protected internal virtual IAsyncResult BeginInvokeActionMethodWithFilters(ControllerContext controllerContext, IList<IActionFilter> filters, ActionDescriptor actionDescriptor, IDictionary<string, object> parameters, AsyncCallback callback, object state) {
            ...
            Invocation<ActionExecutedContext> continuation = new Invocation<ActionExecutedContext>() {
                Begin = (innerCallback, innerState) => BeginInvokeActionMethod(controllerContext, actionDescriptor, parameters, innerCallback, innerState),
                End = ar => new ActionExecutedContext(controllerContext, actionDescriptor, false /* canceled */, null /* exception */) {
                    Result = EndInvokeActionMethod(ar)
                }
            };

            // need to reverse the filter list because the continuations are built up backward
            Invocation<ActionExecutedContext> invocation = filters.Reverse().Aggregate(continuation,
                (next, filter) => new Invocation<ActionExecutedContext>() {
                    Begin = (innerCallback, innerState) => BeginInvokeActionMethodFilter(filter, preContext, next.Begin, next.End, innerCallback, innerState),
                    End = EndInvokeActionMethodFilter
                });
        ...
        private sealed class Invocation<TResult> {
            public BeginInvokeCallback Begin;
            public AsyncCallback<TResult> End;
        }
