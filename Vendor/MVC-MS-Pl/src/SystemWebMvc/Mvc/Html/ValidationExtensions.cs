namespace System.Web.Mvc.Html {
    ...
    public static class ValidationExtensions {
        ...
        public static MvcHtmlString ValidationMessage(this HtmlHelper htmlHelper, string modelName, string validationMessage, IDictionary<string, object> htmlAttributes) {
            ...
            string relativeModelName = modelName;
            modelName = htmlHelper.ViewContext.ViewData.TemplateInfo.GetFullHtmlFieldName(modelName);
            bool clientValidationEnabled = (htmlHelper.ViewContext.FormContext != null && htmlHelper.ViewContext.FormContext.ClientValidationEnabled);

            if (!htmlHelper.ViewData.ModelState.ContainsKey(modelName) && !clientValidationEnabled) {
                return null;
            }
            ...
            if (modelError == null && !clientValidationEnabled) {
                return null;
            }
            ...
            if (clientValidationEnabled) {
                ...
                builder.GenerateId(htmlHelper.ViewContext.FormContext.FormId + "." + modelName + ".validator");
                FieldValidationMetadata fieldMetadata = htmlHelper.ViewContext.FormContext.GetValidationMetadataForField(modelName, true /* createIfNotFound */);
                fieldMetadata.ValidatorId = builder.Attributes["id"];

                // write rules to context object
                ModelMetadata modelMetadata = ModelMetadata.FromStringExpression(relativeModelName, htmlHelper.ViewData);
                IEnumerable<ModelValidator> validators = ModelValidatorProviders.Current.GetValidators(modelMetadata, htmlHelper.ViewContext);
