    ...
    public class DefaultModelBinderTest {
        ...
        public void BindModelValuesCanBeOverridden() {
            ...
            ModelBindingContext bindingContext = new ModelBindingContext() {
                ...
                ValueProvider = new ValueProviderDictionary(null) {
                    ...
                    { "baz", new ValueProviderResult("BazPostValue", "BazPostValue", null) },
                }
            };
        ...
        public void BindModelWithTypeConversionErrorUpdatesModelStateMessage() {
            ...
            ModelBindingContext bindingContext = new ModelBindingContext {
                ...
                ModelName = "",
                ValueProvider = new ValueProviderDictionary(null) {
                    { "IntReadWrite", new ValueProviderResult("foo", "foo", null) },
                },
            };
            DefaultModelBinder binder = new DefaultModelBinder();
        ...
        public void OnModelUpdatedDoesNothingIfModelDoesNotImplementIDataErrorInfo() {
            ...
            // Act
            helper.PublicOnModelUpdated(new ControllerContext(), bindingContext);
        ...
        public void OnPropertyValidatingNotCalledOnPropertiesWithErrors() {
            ...
            ModelBindingContext bindingContext = new ModelBindingContext {
                ...
                ModelName = "",
                ValueProvider = new ValueProviderDictionary(null) {
                    { "foo", new ValueProviderResult("foo", "foo", null) },
                },
            };
        ...
        public void OnPropertyValidatingReturnsFalseAndCreatesValueRequiredErrorIfNecessary() {
            ...
            // Act
            bool returned = helper.PublicOnPropertyValidating(new ControllerContext(), bindingContext, property, null);

            // Assert
            Assert.IsFalse(returned, "Value should not have passed validation.");
            // Temporarily changed for DDB #226223
...
Parameter name: value", bindingContext.ModelState["theModel.BadInt"].Errors[0].Exception.Message);
        ...
        [TestMethod]
        public void OnPropertyValidatingWithValidationAttributeFailing() {
            // Arrange
            TestableDefaultModelBinder<OnPropertyValidatingModel> modelBinder = new TestableDefaultModelBinder<OnPropertyValidatingModel>();

            // Act
            bool result = modelBinder.OnPropertyValidating("RangedInteger", 930);

            // Assert
            Assert.IsFalse(result);
            ModelState modelState = modelBinder.ModelState[BASE_MODEL_NAME + ".RangedInteger"];
            ModelError modelStateErrors = modelState.Errors.Single();
            StringAssert.Contains(modelStateErrors.ErrorMessage, "RangedInteger");
            StringAssert.Contains(modelStateErrors.ErrorMessage, "10");
            StringAssert.Contains(modelStateErrors.ErrorMessage, "65");
        }

        [TestMethod]
        public void OnPropertyValidatingWithRequiredOnValueTypeOnlyResultsInSingleMessage() {    // DDB #225150
            // Arrange
            TestableDefaultModelBinder<OnPropertyValidatingModel> modelBinder = new TestableDefaultModelBinder<OnPropertyValidatingModel>();
            modelBinder.Context.ModelMetadata.Model = null;

            // Act
            bool result = modelBinder.OnPropertyValidating("RequiredInteger", null);
