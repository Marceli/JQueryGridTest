...
namespace Core.Data
{
    ...
    public class SessionProvider:ISessionProvider
    {
        ...
        {
            AutoPersistenceModel model=AutoMap.AssemblyOf<Person>()
                .Where(t => (t.Namespace == "Core.Entities" || t.Namespace == "Core.ValueTypes")).Conventions.AddFromAssemblyOf<Person>()
                .Setup(convention =>
                {
                    convention.IsComponentType =
                        type => type == typeof(Name);
        ...
        {
            if (IsMsqSql())
                return MsSqlConfiguration.MsSql2005.ConnectionString(MSSQLConnectionString).ShowSql().AdoNetBatchSize(10);
            else
